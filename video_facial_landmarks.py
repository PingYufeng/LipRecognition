...
locationCSV = "/Users/sameer/Desktop/Python/FaceRecognition/TabethaBoyajianThemostmysteriousstarintheuniverse.csv"
wordsArray = []
startArray = []
endArray = []
confidenceArray = []
...
with open(locationCSV) as csvfile:
    reader = csv.DictReader(csvfile)
    for column in reader:
        words = column['WORDS']
        start = float(column['START'])
        end = float(column['END'])
        confidence = column['CONFIDENCE']
        wordsArray.append(words)
        startArray.append(start)
        endArray.append(end)
        confidenceArray.append(confidence)
        lengthArray.append(end-start)


with open('manifest.csv', 'wb') as f:
    wr = csv.writer(f, delimiter=' ',quoting=csv.QUOTE_ALL)
    wr.writerow(["start_time,length,rename_to"])
    for i in range(len(wordsArray)):
        row = [""+str(startArray[i])+","+str(lengthArray[i])+","+"video%d" % i]
        wr.writerow(row)

def convertTime(myTime):
        temp = str(datetime.timedelta(seconds=myTime))
        print (temp)
        return temp

for i in range(len(wordsArray)): 
    word = wordsArray[i]
    confidenceNum = confidenceArray[i]

    print(startArray[i])
    print(endArray[i])
    print(type(startArray[i]))
    print(type(endArray[i]))
    
    start = convertTime(startArray[i])
    end = convertTime(endArray[i])
    videoFile = mpy.VideoFileClip("TabethaBoyajianThemostmysteriousstarintheuniverse.mp4").subclip(t_start=start, t_end=end)

    videoFile.write_image_sequences("frame%d.png" % count)
    video = "/Users/sameer/Desktop/Python/FaceRecognition/video%d.mp4/" % i
